...
- **Neovim runtime contract:** the current upstream Kickstart baseline uses `vim.pack` and `PackChanged`; `nvim-custom/lua/custom/plugins/` uses `vim.pack.add`, so Neovim `0.12` or newer is required. If the platform package is older, follow [`How to Upgrade Neovim`](../docs/how-to/neovim-upgrade.md); the shared overlay remains in `~/utils`.
